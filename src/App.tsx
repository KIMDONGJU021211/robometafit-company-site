  ...
  return (
    <>
      ...
      <main id="main">
        ...
        <section className="ontelik-section" aria-labelledby="ontelik-title">
          <div className="ontelik-shell">
            <div className="ontelik-intro" data-reveal>
              <div>
                <p className="section-kicker section-kicker-light">ONTELIK · GOVERNED AI AGENT</p>
                <div className="ontelik-status"><span /> BUILT IN-HOUSE · OPERATIONAL</div>
                <h2 id="ontelik-title">
                  AI가 답하는 순간을 넘어,<br />
                  실제 업무가 <em>끝나는 지점</em>까지.
                </h2>
              </div>
              <div className="ontelik-intro-copy">
                <p>
                  ONTELIK은 사용자의 목표를 이해하고 필요한 도구를 선택해 실제 Chrome·업무 서비스·
                  로컬 자료를 확인하고 실행합니다. 중요한 작업은 사람이 통제하고, 실행 결과는 다시 검증합니다.
                </p>
                <div className="ontelik-tags" aria-label="ONTELIK 핵심 기술">
            ...
            <div className="ontelik-product-stage" data-reveal>
              <figure className="ontelik-shot ontelik-shot-main">
                <div className="ontelik-window-bar">
                  <span className="window-dots"><i /><i /><i /></span>
              ...
              <figure className="ontelik-shot ontelik-shot-browser">
                <span className="ontelik-shot-label">REAL BROWSER EXECUTION</span>
                <img src={`${ASSET}ontelik-browser-agent.png`} alt="ONTELIK Browser Agent가 실제 Chrome에서 사이트를 탐색한 화면" loading="lazy" />
              </figure>

              <figure className="ontelik-shot ontelik-shot-approval">
                <span className="ontelik-shot-label">HUMAN APPROVAL GATE</span>
                <img src={`${ASSET}ontelik-human-approval.png`} alt="중요한 외부 작업 전 ONTELIK이 사용자 승인을 요청하는 실제 화면" loading="lazy" />
              </figure>

              <div className="ontelik-stage-note">
                <span>ACTUAL PRODUCT</span>
                <p>실제 ONTELIK 워크스페이스와 전용 Chrome, 승인 화면을 사용했습니다.</p>
              </div>
            </div>

            <div className="ontelik-architecture" data-reveal>
              <div className="ontelik-architecture-heading">
                <div>
              ...
              <div className="ontelik-entry-points" aria-label="ONTELIK 요청 채널">
                <span>ONTELIK UI</span><i>+</i><span>VOICE</span><i>+</i><span>CLAUDE / CURSOR VIA MCP</span>
              </div>

              <div className="ontelik-flow" aria-label="ONTELIK 작업 흐름">
                {ontelikWorkflow.map(([number, code, label], index) => (
                  <div className="ontelik-flow-step" key={code}>
                    <small>{number}</small>
                    <strong>{code}</strong>
                    <span>{label}</span>
                    {index < ontelikWorkflow.length - 1 && <i aria-hidden="true">→</i>}
                  </div>
                ))}
              </div>

              <div className="ontelik-safety-rail">
                <div>
                  <span>HUMAN GATE</span>
                  <strong>중요한 실행은 화면 승인 후 진행</strong>
                  <p>로그인·비밀번호·보안 확인이 필요하면 사람에게 넘기고, 통과한 지점부터 다시 이어갑니다.</p>
                </div>
                <div className="ontelik-foundation">
                  {["MEMORY", "CHECKPOINT", "AUDIT", "UNDO"].map((item) => <b key={item}>{item}</b>)}
                </div>
              </div>
            </div>

            <div className="ontelik-capability-block">
              <div className="ontelik-block-heading" data-reveal>
              ...
              <div className="ontelik-capabilities">
                {ontelikCapabilities.map(([label, title, description], index) => (
            ...
            <div className="ontelik-principle-block">
              <div className="ontelik-block-heading" data-reveal>
                <p className="section-kicker section-kicker-light">DESIGN PRINCIPLES</p>
                <h3>기능보다 중요한 것은<br />어떻게 믿고 맡길 수 있는가입니다.</h3>
              </div>
              <div className="ontelik-principles">
                {ontelikPrinciples.map(([number, title, description]) => (
            ...
            <div className="ontelik-application" data-reveal>
              <div>
